...
class ReservoirDataset(WsGeoDataset):
    ...
    def _scrape_weekly_reservoir_data(self):
        ...
        if not all_years_reservoir_data.empty:
            all_years_reservoir_data['PCT_OF_CAPACITY'] = pd.to_numeric(all_years_reservoir_data['PCT_OF_CAPACITY'], errors='coerce')
        ...
        all_years_reservoir_data.rename(columns={'StaID': 'STATION_ID', 'Reservoir Name':'RESERVOIR_NAME'}, inplace=True)
    ...
    def _retrieve_stations_geospatial_data(self):
        ...
        """
        all_years_reservoir_data = self.data_df 
    ...
    def preprocess_map_df(self):
        """This function keeps only the STATION_ID, COUNTY and geometry features in the original geospatial data."""
        self.map_df = self.map_df[['STATION_ID', 'LATITUDE', 'LONGITUDE', 'COUNTY','geometry']].drop_duplicates()
